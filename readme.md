...
- [X] 회원가입 기능 (`/register`)  
  - [X] 이메일 중복 체크 기능 추가  
- [X] 로그인 기능 (`/login`) - 세션 기반 인증 
- [X] 로그아웃 기능 (`/logout`)  
- [X] 로그인된 사용자만 접근 가능한 `dashboard` 페이지 
- [X] **비밀번호 변경 (현재 비밀번호 검증 + 새 비밀번호 설정)**
- [X] **OAuth 소셜 로그인 추가 (Google 로그인)**  
- [X] 비밀번호 재설정 기능 (비밀번호 찾기)   
  - [X] **OAuth2 + Nodemailer 연동 (Gmail API 사용)**  
  - [X] 이메일 전송 후 비밀번호 재설정 링크 제공  
  - [X] 비밀번호 재설정 (토큰 검증 후 새 비밀번호 저장)  

---

### **3️⃣ 게시글 CRUD 기능 개발**  
- [X] `Post` 모델 생성 (제목, 내용, 작성자, 생성일)  
- [X] 게시글 작성 기능 (`/posts/new`)  
- [ ] 게시글 조회 기능 (`/posts`)  
  - [X] **조회수 증가 기능 (`views` 필드 추가, 조회 시 증가)**  
  - [X] **정렬 기능 (최신순 / 인기순 / 오래된 순)**  
...
- [ ] **커스텀 오류처리 로직 구현**   (utils/CustomError.js)
---
...
- `uuid` → 비밀번호 찾기 토큰 생성 
- `passport` → 인증 미들웨어 (세션 기반, OAuth 확장 가능)
- `passport-local` → 로컬 로그인 지원 
- `passport-google-oauth20` → Google OAuth2 로그인 처리 (소셜 로그인)
